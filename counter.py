...
class Counter:
    def __init__(self, x1, y1, x2, y2, idx):
        ...
        # the entire ROI including the buffer zone
        width = x2 - x1
        height = y2 - y1
        
        self.big_roi_x1 = x1 - width
        self.big_roi_x2 = x2 + width
        self.big_roi_y1 = y1
        self.big_roi_y2 = y2 + height
        
        self.big_roi_x1 = min(max(0, self.big_roi_x1),1)
        self.big_roi_x2 = min(max(0, self.big_roi_x2), 1)
        self.big_roi_y1 = min(max(0, self.big_roi_y1), 1)
        self.big_roi_y2 = min(max(0, self.big_roi_y2), 1)     
        
        # relative ROI (the actual ROI with respect to big roi)
        big_width = self.big_roi_x2 - self.big_roi_x1
        big_height = self.big_roi_y2 - self.big_roi_y1
        
        relative_width = width / big_width
        relative_height = height / big_height
        
        self.relative_roi_x1 = 0.5 - relative_width / 2
        self.relative_roi_x2 = 0.5 + relative_width / 2
        self.relative_roi_y1 = 0.0
        self.relative_roi_y2 = 0.0 + relative_height
        
        # the actual ROI excluding the buffer zone
        self.roi_x1 = x1
        self.roi_y1 = y1
        self.roi_x2 = x2
        self.roi_y2 = y2
    ...
    def update(self, img_shape=None, pred_boxes=None, reid_dict=None, ref=None):
        ...
        for cls, conf, id, xyxy in zip(all_cls, all_conf, all_id, all_xyxy):
            ...
            # conditions
            condition_1 = x_mid >= self.relative_roi_x1 * img_shape[1]
            condition_2 = x_mid <= self.relative_roi_x2 * img_shape[1]
            condition_3 = y_mid >= self.relative_roi_y1 * img_shape[0]
            condition_4 = y_mid <= self.relative_roi_y2 * img_shape[0]
