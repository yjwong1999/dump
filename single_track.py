...
def main():
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        ...
        while True:
            ...
            current_time = current_time + datetime.timedelta(seconds=30) # library time slower abit
            current_time = current_time.strftime('%H:%M:%S')

            # Display the processed frame
            cv2.putText(result_frame, f"FPS: {fps:.2f}", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2)
            cv2.putText(result_frame, f"Time: {current_time}", (950, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2)
